...
const getGamesByplatform = (platform) => {
    const result = data.filter(item => item.platform.split(" ")[0].toLowerCase() == platform.toLowerCase())
    return result
}
...
export {
    allGames, getPlatforms, getGamesByplatform,
    fetchPlatforms, fetshAllData, fetshGamesByplatFrorm, fetshGameDetails, fetshGamesByTags, fetshSortCartigorySortBy
}